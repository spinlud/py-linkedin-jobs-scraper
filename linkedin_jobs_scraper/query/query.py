...
class QuerySkipOptions(__Base):
    def __init__(self,
                 stop_title_list: list = [],
                 stop_skills_list: list = [],
                 stop_words_description: list = [],
                 stop_company_list: list = [],
                 skip_promoted_jobs: bool = None,
                 stop_location_remote_available: bool = None):
        
        super().__init__()
        
        self.stop_title_list = stop_title_list
        self.stop_skills_list = stop_skills_list
        self.stop_words_description = stop_words_description
        self.stop_company_list = stop_company_list
        self.skip_promoted_jobs = skip_promoted_jobs
        self.stop_location_remote_available = stop_location_remote_available
    
    def validate(self):
        if self.stop_skills_list is not None:
            if not isinstance(self.stop_skills_list, List) or any(
                    [not isinstance(e, str) for e in self.stop_skills_list]):
                raise ValueError('Parameter locations must be a list of strings')
        
        if self.skip_promoted_jobs is not None and not isinstance(self.skip_promoted_jobs, bool):
            raise ValueError('Parameter skip_promoted_jobs must be a boolean')
        
        if self.stop_location_remote_available is not None and not isinstance(self.stop_location_remote_available,
                                                                              bool):
            raise ValueError('Parameter skip_promoted_jobs must be a boolean')
...
class Query(__Base):
    def __init__(self, query: str = '', options: QueryOptions = QueryOptions(),
                 options_skip: QuerySkipOptions = QuerySkipOptions()):
        super().__init__()
        
        self.query = query
        self.options = options
        self.options_skip = options_skip
    ...
    def merge_options(self, options: QueryOptions):
        ...
        if self.options.skip_promoted_jobs is None:
            self.options.skip_promoted_jobs = options.skip_promoted_jobs if options.skip_promoted_jobs is not None \
                else False
