...
class LinkedinScraper:
    ...
    def __init__(
        ...
        if Config.LI_AT_LOGIN:
            info(f'Using strategy {AuthenticatedStrategy.__name__} AT_LOGIN')
            self._strategy = AuthenticatedStrategy(self)
        elif Config.LI_AT_COOKIE:
            info(f'Using strategy {AuthenticatedStrategy.__name__} AT_COOKIE')
            self._strategy = AuthenticatedStrategy(self)
        else:
    ...
    def __build_search_url(query: Query, location: str = '') -> str:
        ...
        if len(location) > 0:
            ...
            params['geoId'] = geoId_dict[location.strip()]
        ...
        if query.options.filters is not None:
            ...
            if query.options.filters.on_site_or_remote is not None:
                if Config.LI_AT_COOKIE or Config.LI_AT_LOGIN:
    ...
    def __run(self, query: Query) -> None:
        ...
        debug('Starting new query', str(query))
        ...
        try:
            ...
            for location in query.options.locations:
                ...
                debug('Websocket debugger url: ', websocket_debugger_url)
    ...
    def on(self, event: Events, cb: Callable, once=False) -> None:
        ...
        if event == Events.DATA or event == Events.DATAFILE or event == Events.ERROR or event == Events.METRICS:
            allowed_params = 1
        else:
            allowed_params = 0
