...
**The ladder is invented, and it stays invented because there is nothing to replace it with.** The obvious improvement is `Retry-After` (RFC 6585 §4), and the Performance API cannot reach it — it exposes statuses, never headers — so the question is really "is the WebDriver BiDi network layer worth wiring in". It was measured on 2026-08-08 rather than argued: **LinkedIn sends no `Retry-After` on a 429 and no `X-RateLimit-*` on anything**. 312 refusals caught passively over BiDi during a deliberately fast run, navigation and XHR alike, carry 17 header names between them and all of them are Cloudflare or LinkedIn routing; six voyager endpoints sampled on normal 200s say the same. The other thing BiDi would buy is per-request attribution, and that was measured too: 292 of those 312 refusals were `voyager/api/…` calls, the remainder three URLs of which one is a tracking beacon — so the counter is already dominated by the signal that matters, and filtering would move it by a few percent. **Do not reopen this without new evidence** (LinkedIn starting to send the header, or Selenium promoting the passive event API to its supported surface and fixing `network.continueResponse`, which currently wedges the browser). `tmp/docs/bidi-network-handoff.md` holds the numbers and `tests/manual/network_headers_probe.py` is the instrument to re-run.
